 ...
 *
 * Author: Mike Chan (mike@android.com) - Original interactive governor
 * Author: Emre Harbutoğlu (coder44278@gmail.com) - Initial cyan port
 * Author: Paul Reioux (reioux@gmail.com) - Modified for cyan
 * Author: Albert I (krascgq@outlook.co.id) - Adaptation to msm-3.10
 *
 */
...
struct cpufreq_cyan_cpuinfo {
	...
	struct cpufreq_cyan_tunables *cached_tunables;
	int first_cpu;
	int prev_load;
	unsigned int two_phase_freq;
};

static DEFINE_PER_CPU(struct cpufreq_cyan_cpuinfo, cpuinfo);
...
struct cpufreq_cyan_tunables {
	...
	*/
	...
	int two_phase_freq_array[NR_CPUS];
...
static struct cpufreq_cyan_tunables *common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_cyan_tunables *tunables)
{
...
static void cpufreq_cyan_timer_resched(unsigned long cpu)
{
	struct cpufreq_cyan_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_cyan_tunables *tunables =
 ...
 * function.
 */
static void cpufreq_cyan_timer_start(
	struct cpufreq_cyan_tunables *tunables, int cpu)
{
	struct cpufreq_cyan_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int freq_to_targetload(
	struct cpufreq_cyan_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_cyan_cpuinfo *pcpu,
...
static void cpufreq_cyan_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_cyan_cpuinfo *pcpu =
	...
	int i, max_load;
	unsigned int max_freq;
	struct cpufreq_cyan_cpuinfo *picpu;
	...
	if (tunables->use_sched_load) {
		 ...
		 * with cpufreq_cyan_timer_resched/start().
	...
	if (cpu_load >= tunables->go_hispeed_load || boosted) {
		if (pcpu->policy->cur < tunables->hispeed_freq) {
			...
			pcpu->two_phase_freq = tunables->two_phase_freq_array[nr_cpus-1];
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_cyan_timer_resched(data);
...
static void cpufreq_cyan_idle_start(void)
{
	struct cpufreq_cyan_cpuinfo *pcpu =
	...
	if (pcpu->target_freq != pcpu->policy->min) {
		...
		if (!pending) {
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_cyan_timer_resched(smp_processor_id());
...
static void cpufreq_cyan_idle_end(void)
{
	struct cpufreq_cyan_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		del_timer(&pcpu->cpu_timer);
		del_timer(&pcpu->cpu_slack_timer);
		cpufreq_cyan_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_cyan_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			struct cpufreq_cyan_cpuinfo *pjcpu;
			u64 hvt;
...
static void cpufreq_cyan_boost(void)
{
	int i;
	int anyboost = 0;
...
static int cpufreq_cyan_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_cyan_notifier,
};
...
static ssize_t show_two_phase_freq(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	int i = 0 ;
	int shift = 0 ;
	char *buf_pos = buf;
	for ( i = 0 ; i < NR_CPUS; i++) {
		shift = sprintf(buf_pos,"%d,",tunables->two_phase_freq_array[i]);
		buf_pos += shift;
	}
	*(buf_pos-1) = '\0';
	return strlen(buf);
...
static ssize_t store_two_phase_freq(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	...
	if (NR_CPUS == 1)
		ret = sscanf(buf, "%u",
				&tunables->two_phase_freq_array[0]);
	else if (NR_CPUS == 2)
		ret = sscanf(buf, "%u,%u",
				&tunables->two_phase_freq_array[0],
				&tunables->two_phase_freq_array[1]);
	else if (NR_CPUS == 4)
		ret = sscanf(buf, "%u,%u,%u,%u",
				&tunables->two_phase_freq_array[0],
				&tunables->two_phase_freq_array[1],
				&tunables->two_phase_freq_array[2],
				&tunables->two_phase_freq_array[3]);
	else if (NR_CPUS == 6)
		ret = sscanf(buf, "%u,%u,%u,%u,%u,%u",
				...
				&tunables->two_phase_freq_array[2],
				&tunables->two_phase_freq_array[3],
				&tunables->two_phase_freq_array[4],
				&tunables->two_phase_freq_array[5]);
	else if (NR_CPUS == 8)
		ret = sscanf(buf, "%u,%u,%u,%u,%u,%u,%u,%u",
				...
				&tunables->two_phase_freq_array[4],
				&tunables->two_phase_freq_array[5],
				&tunables->two_phase_freq_array[6],
				&tunables->two_phase_freq_array[7]);
	if (ret < NR_CPUS)
		return -EINVAL;
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_cyan_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_sampling_down_factor(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->sampling_down_factor);
}

static ssize_t store_sampling_down_factor(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
static ssize_t show_go_hispeed_load(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_min_sample_time(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_cyan_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_cyan_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_cyan_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t store_boostpulse(struct cpufreq_cyan_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boostpulse_duration(struct cpufreq_cyan_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_cyan_tunables
...
static ssize_t show_io_is_busy(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_sync_freq(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->sync_freq);
}

static ssize_t store_sync_freq(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_up_threshold_any_cpu_load(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", tunables->up_threshold_any_cpu_load);
}

static ssize_t store_up_threshold_any_cpu_load(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_up_threshold_any_cpu_freq(struct cpufreq_cyan_tunables *tunables,
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", tunables->up_threshold_any_cpu_freq);
}

static ssize_t store_up_threshold_any_cpu_freq(struct cpufreq_cyan_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_cyan_enable_sched_input(
			struct cpufreq_cyan_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_cyan_tunables *t;
...
static int cpufreq_cyan_disable_sched_input(
			struct cpufreq_cyan_tunables *tunables)
{
	mutex_lock(&sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_cyan_enable_sched_input(tunables);
	else
		ret = cpufreq_cyan_disable_sched_input(tunables);
...
static struct attribute *cyan_attributes_gov_sys[] = {
...
static void cyan_input_event(struct input_handle *handle,
		unsigned int type,
...
static int cyan_input_connect(struct input_handler *handler,
...
static void cyan_input_disconnect(struct input_handle *handle)
{
...
static const struct input_device_id cyan_ids[] = {
	{ .driver_info = 1 },
	{ },
};

static struct input_handler cyan_input_handler = {
	.event		= cyan_input_event,
	.connect	= cyan_input_connect,
	.disconnect	= cyan_input_disconnect,
	.name		= "cyan",
	.id_table	= cyan_ids,
};

static struct attribute_group cyan_attr_group_gov_sys = {
	.attrs = cyan_attributes_gov_sys,
	.name = "cyan",
};

/* Per policy governor instance */
static struct attribute *cyan_attributes_gov_pol[] = {
...
static struct attribute_group cyan_attr_group_gov_pol = {
	.attrs = cyan_attributes_gov_pol,
	.name = "cyan",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &cyan_attr_group_gov_pol;
	else
		return &cyan_attr_group_gov_sys;
}

static int cpufreq_cyan_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_cyan_idle_start();
		break;
	case IDLE_END:
		cpufreq_cyan_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_cyan_idle_nb = {
	.notifier_call = cpufreq_cyan_idle_notifier,
};
...
static struct cpufreq_cyan_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_cyan_tunables *tunables;
	...
	tunables->up_threshold_any_cpu_load = DEFAULT_UP_THRESHOLD_ANY_CPU_LOAD;
	tunables->sync_freq = DEFAULT_SYNC_FREQ;
	tunables->up_threshold_any_cpu_freq = DEFAULT_UP_THRESHOLD_ANY_CPU_FREQ;
	tunables->two_phase_freq_array[NR_CPUS] = DEFAULT_TWO_PHASE_FREQ_ARRAY;
...
static struct cpufreq_cyan_tunables *restore_tunables(
						struct cpufreq_policy *policy)
{
	int cpu;
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!policy->governor->initialized) {
			idle_notifier_register(&cpufreq_cyan_idle_nb);
			cpufreq_register_notifier(&cpufreq_notifier_block,
					CPUFREQ_TRANSITION_NOTIFIER);
		}

		if (tunables->use_sched_load)
			cpufreq_cyan_enable_sched_input(tunables);
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		if (!--tunables->usage_count) {
			if (policy->governor->initialized == 1) {
				cpufreq_unregister_notifier(&cpufreq_notifier_block,
						CPUFREQ_TRANSITION_NOTIFIER);
				idle_notifier_unregister(&cpufreq_cyan_idle_nb);
			}
		...
		if (tunables->use_sched_load)
			cpufreq_cyan_disable_sched_input(tunables);
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_cyan_timer_start(tunables, j);
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!policy->cpu)
			input_unregister_handler(&cyan_input_handler);
...
static void cpufreq_cyan_nop_timer(unsigned long data)
{
}
...
static int __init cpufreq_cyan_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_cyan_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_cyan_nop_timer;
	...
	speedchange_task =
		kthread_create(cpufreq_cyan_speedchange_task, NULL,
			       "cfcyan");
...
static void __exit cpufreq_cyan_exit(void)
{
	int cpu;
	struct cpufreq_cyan_cpuinfo *pcpu;
...
module_exit(cpufreq_cyan_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_AUTHOR("Emre Harbutoğlu <coder44278@gmail.com>");
MODULE_AUTHOR("Paul Reioux <reioux@gmail.com>");
MODULE_AUTHOR("Albert I <krascgq@outlook.co.id>");
